...
## ✅ Rendered-artifact verification (PASS 2b — coordinated restart, 2026-07-14)
Jake paused his walk; I restarted the dev server on a clean `.next` (cleared the corrupt cache) and render-verified all three on the healthy server — **each reads $22,983.20 (canonical), not the stale $23,623.20:**
1. **Cover letter** — GET `/api/draws/[B]/cover-letter` body: *"Current Payment Due: $22,983.20"* (+ Contract $500,000.00, Completed $26,248.00 (5.2%), Retainage $2,624.80).
2. **Excel export** — generated xlsx, sheet "Project Summary (G702)", cell **G23 = 22,983.20** (ladder: Original 500,000 · Completed 26,248 · Less-Prev 0 · Balance 476,376.80). *Also fixed a pre-existing export blocker (below).*
3. **Owner portal** — `/owner/{fixture-token}/pay-apps/[B]` (flag on, temp token — both reverted): *"Work completed this period: $22,983.20"*, Remaining $476,376.80, Paid-before $0.00, Total $500,000.00.
4. **App health** — pay-app detail page renders clean on the final flag-off server (CURRENT PAYMENT DUE $22,983.20 + 3-line G703 + −$640 credit); Jake's walk resumes on a healthy server.

**Export filename fix (pre-existing, non-money):** the Excel export 500'd on `Content-Disposition` for any job whose name has a non-ASCII char (the em-dash in "Sample Client B — 456 Demo Ave" → ByteString error). Sanitized the filename to ASCII (`replace(/[^\x20-\x7E]/g, "-")`). Surfaced by the NEW-2 export verify; not draw math.

**Stale test #2 (out-of-class, adjudicated stale — not a regression):** `job-phase-contract-type.test.ts` 2 assertions failed because (3.4) `/jobs/new` is now a redirect to the New-Job slide-over and (2.4) `OnboardWizard.tsx` is gone (contract_type is per-job, split from billing_method). Rewrote both to current deliberate policy (slide-over `contract_type` + `billing_method` split; default `cost_plus_aia`; the quick-create control curates to 3 common types per Pattern 4, DB/API still support all 6). **Full suite now 100% green — no known-reds.**

**Env note:** the `.next` corruption traced to session-1's authorized `:3100` kill on the shared cache (Lesson 12 in `.planning/lessons.md`). Fixture writes for the owner-portal verify (one temp token + the `NEXT_PUBLIC_FEATURE_OWNER_PORTAL` flag) were fully reverted; server restarted clean.
