...
authorization: nwrp202 RE-APPROVE + dispatch authorization (chain nwrp200 → nwrp201 → nwrp202)
source_decisions:
...
files_referenced:
  ...
  - supabase/migrations/00038_phase_b_internal_billings.sql   # composite UNIQUE precedent #3
...
6. **Index posture eliminating timing oracle** — full covering index `(org_id, client_id, job_id, revoked_at, expires_at)` instead of partial `WHERE revoked_at IS NULL` (partial index makes revoked-token lookup miss the index, causing measurable seq-scan delay distinguishing "revoked" from "never existed"). App-layer filters apply `revoked_at IS NULL AND expires_at > now()`. (Resolves iter-1 SYNTHESIS B-7; NO Jake-acceptance partial escape per nwrp202 §4.)
...
### Plan-author latitude (NOT pre-decided in this PLAN; surface to iter-1 if material)
...
#### Plan-Author Latitude #3 — `revoked_seq` column + re-invitation pattern (ITER-1 DISPOSITION REQUIRED per BLOCKER #3)

**Background.** CONTEXT.md D-04 LOCKED the index posture: full covering index `(org_id, client_id, job_id)` + app-layer filter on `revoked_at` + `expires_at`. D-04 did NOT lock a re-invitation pattern. The full covering index without `WHERE revoked_at IS NULL` partial predicate eliminates the timing oracle (per iter-1 SYNTHESIS B-7 + nwrp202 §4), but creates a re-invitation problem: an existing revoked row's `(org_id, client_id, job_id)` triplet still occupies the uniqueness slot unless the predicate carries a discriminator. Plan-author surfaces 3 options for iter-1 disposition; default is Option A.

**Option A (plan-author default; embedded in §5 Task 1 §1e + AC-B2a-08).** Add `revoked_seq INTEGER NOT NULL DEFAULT 0` column. Unique constraint: `(org_id, client_id, job_id, revoked_seq) WHERE client_id IS NOT NULL`. On revoke: increment `revoked_seq` to (max+1) for the row. Re-invitation creates a NEW row with `revoked_seq = 0` (collision avoided because old row's seq was bumped to ≥1). Row identity survives re-invitation; full audit trail of every revocation cycle preserved.
...
**Option B.** Soft-delete-on-revoke + hard cascade on re-invite. No new column. Revocation sets `revoked_at` timestamp; the row is conceptually "tombstoned." On re-invitation, the existing revoked row is HARD-DELETED (`DELETE FROM client_portal_access WHERE id = ... AND revoked_at IS NOT NULL`) immediately before the RPC INSERT, freeing the unique slot.
- **Pros:** Zero schema change beyond what's already in §5 Task 1; no new column.
- **Cons:** Violates CLAUDE.md "Never delete records — soft delete only" Dev Rule (the activity_log row recording the revocation remains, but the token row itself is destroyed); breaks audit-log foreign-key chain via `activity_log.actor_token_id REFERENCES client_portal_access(id) ON DELETE SET NULL` (the FK fires SET NULL, severing the link to the revoked-then-deleted token); loses row identity across re-invitation cycles (admin UI cannot show "this client was previously revoked + re-invited 3 times").

**Option C (compromise).** Predicate the unique constraint with `revoked_at IS NULL`. Existing 00074:181-185 pattern (`client_portal_access_org_job_email_active`) does this for `(org_id, job_id, email)`. Re-invitation works naturally — revoked rows leave the unique scope.
...
- **Cons:** Re-introduces the timing oracle (revoked-token lookups miss the partial-predicate-narrowed index → seq-scan latency distinguishes revoked from never-existed). NOT compatible with iter-1 SYNTHESIS B-7 + nwrp202 §4 (NO partial-index escape per D-04 + nwrp202 §4 explicit prohibition). Rejected at plan-author level; surfaced for completeness only.

**Plan-author default: Option A.** Reviewers (security + database + multi-tenant-architect) may select B if they accept the soft-delete-rule exception trade for the schema simplicity; rejecting C per nwrp202 §4 is mechanical.

**Concrete consequence of Option B/C selection at iter-1:** if iter-1 approves Option B, §5 Task 1 §1e (the `revoked_seq` column add) is REMOVED, §5 Task 1 §1f index predicate becomes `(org_id, client_id, job_id) WHERE client_id IS NOT NULL` (no `revoked_seq`), and AC-B2a-08 references the soft-delete-then-cascade pattern instead of `revoked_seq` increment. If iter-1 approves Option C, §5 Task 1 §1f predicate becomes `(org_id, client_id, job_id) WHERE client_id IS NOT NULL AND revoked_at IS NULL` and AC-B2a-05 timing-oracle verification is OBSOLETED + Jake explicit acceptance required. Plan-author is committing to Option A as default; iter-1 reviewers MUST explicitly confirm OR pick B/C with rationale.

#### Plan-Author Latitude #4 — sliding-window UPDATE 90-day → 1-year in 2 RPCs (ITER-1 DISPOSITION REQUIRED per BLOCKER #2)
...
**Plan-author CHOICE (default):** in-scope. Author both RPC bodies inline in §5 Task 1 §3b + §3c (already done per BLOCKER #2 fix). Change the single sliding-window-UPDATE-interval line in each RPC from `'90 days'` to `'1 year'`. Preserve everything else byte-for-byte (auth checks, `SET search_path = public, pg_temp`, silent-no-op patterns, message insert path).

**Alternative (rejected by plan-author; iter-1 may override):** defer the two RPC sliding-window changes to a follow-up plan. Accept inconsistent semantics post-B-2a ship: any token use rolls the window back to 90 days, contradicting the nwrp200 1-year lock. iter-1 reviewers (security + database + ai-logic-tester) must EXPLICITLY confirm in-scope OR override to defer.

**Net scope:** ~30 lines of SQL inside the migration body (full RPC re-author for 2 RPCs). Cost-impact: zero — this is the NECESSARY CONSEQUENCE of nwrp200 1-year LOCKED, not net new scope. ai-logic-tester verifies at iter-1 by reading the post-edit RPC bodies and confirming the only material diff against 00074 is the single sliding-window-interval line per RPC.
...
| **B-4** | rls-auditor W-4 | Existing `create_client_portal_invite` RPC produces `client_id=NULL` → PostgREST `.eq('client_id', null)` matches ALL rows | `CREATE OR REPLACE FUNCTION` derives `client_id` from `jobs.client_id` inside function body + RAISE if NULL (§5 Task 1 sub-step e); 3 falsifiable queries per AC-B2a-02 | D-05 + D-06 |
...
| **B-7** | security BLOCKING-2 | Partial index `WHERE revoked_at IS NULL` creates timing oracle (revoked-token lookup misses index → seq-scan delay distinguishes "revoked" from "never existed") | Full covering index `(org_id, client_id, job_id, revoked_at, expires_at)` + app-layer `revoked_at IS NULL` filter (§5 Task 1 sub-step d); NO Jake-acceptance partial escape per nwrp202 §4 | D-04 |
| **B-8** | ai-logic BLOCKING-1 | PostgREST FK citation missing | `client_portal_access_client_id_fkey` (composite) + `client_portal_access_job_id_fkey` (inline FK on 00074:152) cited in §5 Task 1 sub-step b + AC-B2a-09 | D-19 |
| **B-9** | database BLOCKING-1 + ai-logic NOTE | Partial unique index `WHERE revoked_at IS NULL` doesn't exclude expired-but-not-revoked tokens — expired token occupies `(org_id, client_id)` unique slot, blocks re-invitation | Predicate extended: `WHERE revoked_at IS NULL AND client_id IS NOT NULL AND expires_at > now()` (§5 Task 1 sub-step d) | D-03 |
| **B-10** | ai-logic NOTE-2 | Partial unique `(org_id, client_id)` blocks 1-client-N-jobs (a single homeowner with 2 jobs cannot have 2 tokens) | Composite key extended to `(org_id, client_id, job_id)` (§5 Task 1 sub-step d) | D-03 |
| **B-11** | database BLOCKING-2 | `draws` + `change_orders` `job_id` indexes not verified — first production queries from external path | `draws.job_id` index added in 00104 (does NOT exist today); `change_orders.job_id` verified extant at 00028:171 (§5 Task 1 sub-step i) | D-18 |
...
| Precedent in codebase | 4 established (00016:104, 00019:17, 00038:41, 00083:30) | Zero — would be novel anti-pattern |
...
**Decision: full covering index `(org_id, client_id, job_id)` + app-layer filter on `revoked_at` and `expires_at`. REJECT partial-index escape per nwrp202 §4 (NO Jake-acceptance partial allowed).**
...
CREATE UNIQUE INDEX client_portal_access_org_client_job_active
  ON public.client_portal_access (org_id, client_id, job_id)
...
The UNIQUE index supports the 1-client-N-jobs rule (iter-1 SYNTHESIS B-10): a single client can have tokens for 2 different jobs in the same org (different `job_id`s allowed; same `(org_id, client_id, job_id)` triplet blocked). It is conditional on `client_id IS NOT NULL` because pre-backfill rows could (in rare cases) carry NULL `client_id` until the backfill UPDATE applies — though our forward probe at Step 4 rejects this state. Post-migration, the predicate excludes the (impossible-but-defended) NULL `client_id` rows from uniqueness. Predicate intentionally OMITS `revoked_at IS NULL` per the timing-oracle elimination — app-layer enforces revocation filtering. Re-invitation after revocation: app-layer revoke first (set `revoked_at = NOW()`) then RPC creates a new row; the NEW row collides with the OLD row on `(org_id, client_id, job_id)` unique constraint UNLESS the OLD row had `revoked_at` set — but since the unique index does NOT exclude revoked rows, the OLD row continues to occupy the uniqueness slot. **TRADEOFF:** the timing-oracle elimination via full index forces a different re-invitation pattern — revoke + then DELETE the row, OR carry a `revoked_seq` column. **Plan-author surfaces this to iter-1 reviewers: which pattern wins?** The existing 00074:181-185 unique index DOES exclude `revoked_at IS NULL`; that pattern carries over for re-invitation IF the security-reviewer accepts the timing-oracle tradeoff. Iter-1 disposition options:

- **Option A (chosen pre-iter-1):** unique predicate excludes `client_id IS NULL` only; revocation appends to history (no re-invitation collision because new row has `(org_id, client_id, job_id)` but old row also has same triplet — VIOLATION). This is the strict full-covering-no-timing-oracle posture; it requires `revoked_seq` integer column OR a soft-delete approach. **Adds scope.** Surface to iter-1.
- **Option B:** unique predicate includes `revoked_at IS NULL` (the existing 00074 pattern). This re-introduces the timing oracle. NOT compatible with iter-1 SYNTHESIS B-7 + nwrp202 §4.
- **Option C (compromise):** unique predicate excludes `revoked_at IS NULL` AND `expires_at > now()`. Same partial-index timing pattern but with two filters; same timing oracle exposure.

**Plan-author proposes Option A + adding `revoked_seq INTEGER NOT NULL DEFAULT 0` to `client_portal_access`** — uniqueness becomes `(org_id, client_id, job_id, revoked_seq)`. On revocation, the row's `revoked_at` is set + `revoked_seq` is set to (max + 1). Re-invitation creates a NEW row with `revoked_seq = 0` (unique slot freed because no other row has `(triple, 0)` once the old row's seq was bumped to 1). This is the "revoke + increment" pattern; clean; preserves audit trail; no timing oracle; minimal scope add (~5 lines of migration + 2 lines of admin revoke route).

**iter-1 reviewer plan-disposition:** plan-author surfaces Option A + revoked_seq column. Security-reviewer + database-reviewer verify the index + predicate behavior matches the no-timing-oracle requirement. Multi-tenant-architect verifies the BY-CONSTRUCTION posture preserved.
...
Existing rows are NOT affected (a `SET DEFAULT` change only affects future INSERTs). Existing tokens retain their original `expires_at`; new tokens get 1-year. Sliding-window UPDATE behavior on token use (00074:489) preserved unchanged — extends `expires_at` to `now() + interval '90 days'` regardless of original default. **Plan-author surfaces:** should sliding-window UPDATE also extend to 1-year? Per nwrp200 LOCKED + iter-1 reviewer guidance: yes; sliding-window extension is changed to `now() + interval '1 year'` in the relevant RPC. **`submit_client_portal_message` at 00074:489 and `mark_client_portal_message_read` at 00074:540** both contain sliding-window UPDATEs — both need the 90-day → 1-year change in `CREATE OR REPLACE` re-definitions. **Plan-Author Latitude #4 (see §2) formally surfaces this for iter-1 disposition.** Plan-author CHOICE (default): in-scope. Both RPCs are SECURITY DEFINER; both contain the same one-line change. Adds ~10 lines net. No new test surface (the existing RPCs are not yet called from `src/`). The migration is atomic per D-17; the 1-year default must be consistent across all sliding-window UPDATEs OR be explicitly documented as inconsistent.
...
1. Middleware allows (PUBLIC_PATHS includes `/api/owner-portal/*`).
2. Route handler calls `getCurrentMembership()` — NOT bypassed by PUBLIC_PATHS extension; PUBLIC_PATHS only allows auth-redirect-skip, not auth-check-skip.
3. Assert `membership.role IN ('owner', 'admin')`. PM role NOT allowed (admin function only). 403 otherwise.
4. Call `updateWithLock<{ id: string; updated_at: string }>(supabase, { table: 'client_portal_access', id: token_id, orgId: membership.org_id, expectedUpdatedAt: expected_updated_at, updates: { revoked_at: 'now()' as any /* Supabase casts string→now()*/, revoked_seq: <max+1> } })` — uses optimistic-lock per D-11.
5. On success: write audit-log row via Sweep 4 Open Q resolution (plan-author proposes the `ActivityEntityType` extension to include `'client_portal_access'`; row written with `user_id = membership.user_id`, `actor_token_id = NULL`, `entity_type = 'client_portal_access'`, `entity_id = token_id`, `action = 'revoked'` — but `'revoked'` is NEW; the existing union has `'voided'`/`'denied'`. **Plan-author proposes adding `'revoked'` to `ActivityAction` union** + Record entry; ~3 lines of TS in `src/lib/activity-log.ts`).
6. Return 200 + `{ revoked: true, revoked_at: now_iso }`. On `isLockConflict`: return the conflict response from `updateWithLock`.
 ...
 *
 * Per iter-1 SYNTHESIS B-7 + nwrp202 §4: revocation + expiration filters
 * applied app-layer, NOT via partial index, to eliminate timing oracle.
 *
...
export async function recordOwnerPortalRequest(
  ...
  const { data, error } = await supabase
    ...
    .upsert(
      ...
      { onConflict: "key_type,key_value,window_start", ignoreDuplicates: false }
    )
  ...
  // Increment via separate UPDATE (upsert with ON CONFLICT DO UPDATE)
  // — the above .upsert with `ignoreDuplicates: false` re-inserts then
  // returns existing row; we need explicit increment. Alternative pattern:
  // call a RPC `increment_owner_portal_request(key_type, key_value)` that
  // returns the new count via INSERT ... ON CONFLICT DO UPDATE SET
  // request_count = request_count + 1 RETURNING request_count.
  // Plan-author proposes the RPC approach for atomicity; surface to iter-1.
  // (Pseudo-code for the RPC-based approach shown in §5 Task 1 sub-step h.)
...
Single-line addition to `src/middleware.ts:7`:
...
const PUBLIC_PATHS = [
  ...
  "/owner",          // B-2a: Owner Portal Path A — anon homeowner UI (consumed by B-2b)
  "/api/owner-portal" // B-2a: Owner Portal Path A — anon homeowner API + admin revocation API
];
```

The existing `isPublic` matcher (lines 30-35) handles `/owner/{token}` (matches `/owner` prefix) AND `/api/owner-portal/admin/revoke-token` (matches `/api/owner-portal` prefix). Per Sweep 3: PUBLIC_PATHS only bypasses the auth-redirect; route handlers (admin revoke, B-2b acknowledge) MUST do their own auth checks via `getCurrentMembership()` or `resolveOwnerToken()`.
...
- `draws.job_id` index: DOES NOT exist. No matching CREATE INDEX statement found across all migrations.

B-2a migration 00104 adds the missing index:

```sql
-- Per iter-1 SYNTHESIS B-11 + D-18: draws.job_id index does NOT exist today;
-- first user-facing portal query path would seq-scan the table.
...
| T-B2a-05 | I | Timing oracle distinguishing revoked vs never-existed tokens | mitigate | Full covering index + app-layer filter on `revoked_at` + `expires_at` (eliminates index-miss timing per D-04); NO partial-index escape |
...
| T-B2a-12 | E | SECURITY DEFINER function PUBLIC EXECUTE inheritance | mitigate | `REVOKE EXECUTE ... FROM PUBLIC` on new SECURITY DEFINER functions (per 00103 precedent); explicit GRANT to `service_role` only |
...
**Action:** atomic single-migration containing 9 logical sub-steps per D-17. Pre-flight DO block verifies fixture-harness-org + Drummond baselines; forward probe DO block raises EXCEPTION if backfill leaves orphans; final NOTIFY pgrst.
...
-- §1e — revoked_seq column for re-invitation pattern.
--        CONDITIONAL ON Plan-Author Latitude #3 Option A approval at iter-1
--        (see §2 Plan-Author Latitude #3 for Options A/B/C tradeoff matrix).
--        If iter-1 approves Option A (plan-author default): keep this block as authored.
--        If iter-1 approves Option B: REMOVE this block; §1f predicate becomes
--          `(org_id, client_id, job_id) WHERE client_id IS NOT NULL` (no `_seq`).
--        If iter-1 approves Option C: REMOVE this block; §1f predicate becomes
--          `(org_id, client_id, job_id) WHERE client_id IS NOT NULL AND revoked_at IS NULL`
--          (re-introduces partial; requires Jake explicit acceptance per nwrp202 §4).
ALTER TABLE public.client_portal_access
  ADD COLUMN revoked_seq INTEGER NOT NULL DEFAULT 0;

-- §1f — Indexes (per D-03 + D-04 timing-oracle-elimination posture).
--        Predicate shape DEPENDS on Plan-Author Latitude #3 Option choice (see §1e header).
--        Below = Option A shape (with `revoked_seq` in unique tuple).
...
--       Authored inline per BLOCKER #2 (Plan-Author Latitude #4 ITER-1 DISPOSITION REQUIRED — see §2).
...
--       00074:501-543 (sliding-window UPDATE at 00074:538-541). Authored inline per BLOCKER #2.
...
# AC-B2a-01..09 verification queries (§8 below)
```

**Done:** Migration applies cleanly; forward probe passes; types regenerated; 9 sub-step verification queries return expected results per AC-B2a-01..09.

**Atomic commit:** `feat(b-2a): migration 00104 — composite FK + RPC NULL-leak fix + 1-year + rate-limit + actor_token_id + draws index`
...
**Action:** single-line edit at line 7 per §6.g. Add `"/owner"` and `"/api/owner-portal"` to the PUBLIC_PATHS array.

**Verify:**
```bash
# Manual: hit /owner/abc as anon — middleware allows (no auth redirect to /login).
#         hit /api/owner-portal/foo as anon — middleware allows.
#         hit /dashboard as anon — middleware redirects to /login (PUBLIC_PATHS extension does NOT regress auth).
npm run typecheck
```

**Done:** PUBLIC_PATHS extension does NOT regress auth for non-owner paths; new paths bypass auth-redirect (route handlers will do their own auth check).

**Atomic commit:** `feat(b-2a): middleware PUBLIC_PATHS extension (/owner + /api/owner-portal)`
...
2. Call `getCurrentMembership()` — return 401 if not authenticated, 403 if role NOT IN ('owner', 'admin').
3. Call `recordOwnerPortalRequest('ip', requestIp)` — return 429 if rate-limited.
4. Call `updateWithLock<{ id: string; updated_at: string }>(supabase, { table: 'client_portal_access', id: tokenId, orgId: membership.org_id, expectedUpdatedAt, updates: { revoked_at: new Date().toISOString(), revoked_seq: <max+1 sub-query> } })`.
5. On success: call `logActivity({ org_id: membership.org_id, user_id: membership.user_id, actor_token_id: null, entity_type: 'client_portal_access', entity_id: tokenId, action: 'revoked', details: { revoked_at: <iso> } })`.
6. Return 200 + `{ revoked: true, revoked_at: <iso> }`. On `isLockConflict`: return the conflict response.
...
curl -X POST .../revoke-token -d '{...}' -H "Cookie: <pm-session>"
# Expect: 403.
```

**Done:** Admin can revoke; audit-log written; cross-tenant probe blocked by RLS-scoped `updateWithLock`; PM role blocked by route-handler role check.

**Atomic commit:** `feat(b-2a): admin revocation API route (/api/owner-portal/admin/revoke-token)`
...
## 8. Acceptance criteria (13 falsifiable items)

Per EXPANDED-SCOPE §7 acceptance criteria preview lines 393-406 + B-4 RPC fix AC strengthened per nwrp201 + nwrp202 §5 to 3 falsifiable queries. Each AC has explicit verification query in §9.
...
- **AC-B2a-04** — Down-migration `00104_*.down.sql` cleanly reverses; restored RPC body matches 00074:390-439 byte-for-byte; idempotent on re-apply (DROP IF EXISTS).
...
- **AC-B2a-07** — Token expiration LOCKED to 1-year (supersedes 00074 90-day): `client_portal_access.expires_at` column default = `(now() + interval '1 year')`; RPC fallback in `create_client_portal_invite` body = `now() + interval '1 year'`. Sliding-window UPDATE in `submit_client_portal_message` + `mark_client_portal_message_read` extends to 1-year (per Plan-Author Latitude #4). Migration body documents supersession.
- **AC-B2a-08** — Admin revocation end-to-end: `POST /api/owner-portal/admin/revoke-token` with `{ token_id, expected_updated_at }` from authenticated admin (`owner|admin` role) returns 200; `client_portal_access.revoked_at` populated; `activity_log` row written with `user_id = membership.user_id`, `actor_token_id = NULL`, `entity_type = 'client_portal_access'`, `action = 'revoked'`. Admin UI at `/admin/owner-portal-tokens/page.tsx` renders the action affordance; clicking Revoke triggers the API call and updates the row state. **Re-invitation discriminator depends on Plan-Author Latitude #3 disposition:** **Option A default:** `revoked_seq` column incremented to (max+1) on revoke. **Option B (if iter-1 selects):** revoked row hard-deleted at next re-invitation INSERT; AC verifies hard-delete sequence + activity_log row preservation post-cascade. **Option C (if iter-1 selects per nwrp202 §4 exception):** partial-index predicate `WHERE revoked_at IS NULL` re-introduced; timing-oracle AC-B2a-05 obsoleted + Jake explicit acceptance noted in SUMMARY.

**Middleware + rate-limit + helpers (3 ACs):**

- **AC-B2a-09** — Middleware PUBLIC_PATHS extension: `/owner/*` AND `/api/owner-portal/*` reach route handlers as anon (NOT 401 by middleware). Route handlers retain their own auth checks (admin revoke requires authenticated admin via `getCurrentMembership()`; B-2b acknowledge requires token resolution). PostgREST FK citations: `client_portal_access_client_id_fkey` (single-col) + `client_portal_access_org_id_client_id_fkey` (composite) + `client_portal_access_job_id_fkey` (from 00074:152) referenced in plan body per Workflow Rule 2 (closes iter-1 SYNTHESIS B-8).
...
- **AC-B2a-13 (GATE B-2a)** — Jake reviews + confirms token scoping is airtight via 8 EXPANDED-SCOPE §9 verification items BEFORE B-2b dispatches:
  ...
  - (8) PostgREST FK citations match `pg_constraint`; `draws.job_id` + `change_orders.job_id` indexes verified (AC-B2a-09 + AC-B2a-12).
  - (9) **Drummond token row backfill verified — canary for D-02 per CONTEXT.md `<specifics>` (per WARNING #3 / iter-1 plan-checker).** Direct SQL probe: `SELECT cpa.client_id, j.client_id AS jobs_client_id FROM public.client_portal_access cpa JOIN public.jobs j ON j.id = cpa.job_id WHERE j.name = 'Drummond';` returns `cpa.client_id = j.client_id` for every Drummond `client_portal_access` row (zero-divergence). Drummond is the reference job for production-facing artifacts (CLAUDE.md Domain rules); D-02 explicitly designates the Drummond row backfill as the canary because if Drummond breaks, every downstream user-facing path breaks visibly to Jake.
...
### AC-B2a-02 — Composite FK invariant
...
# Timing-oracle test (Layer 5 ai-logic-tester):
# Create test rows: 1 valid + 1 revoked + 1 never-existed (no row at all)
# Repeatedly query each; measure response time variance.
...
**Query 2 — RPC produces non-NULL client_id:**

```bash
mcp__supabase__execute_sql:
  -- Invoke RPC via service-role with a known test job + email
...
# are also updated to 1-year (per Plan-Author Latitude #4 in §2; iter-1 disposition required).
```
...
grep -c "client_portal_access_client_id_fkey\|client_portal_access_org_id_client_id_fkey\|client_portal_access_job_id_fkey" \
...
#   draws: idx_draws_job_id (NEW B-2a)
```

### AC-B2a-13 — GATE B-2a Jake review

Verification consists of the 8 EXPANDED-SCOPE §9 HALT GATE B-2a items, each cross-referenced to a prior AC (above). Jake reviews + ATTESTS each via SUMMARY.md table at ship time. Rejection → iter-2 OR fresh-session re-scope.
...
|---|---|---|
| Migration 00104 forward + down + types regen | $5-7 | 9 sub-steps; atomic per D-17; types pipeline mandatory |
...
| Admin revocation API + UI surface | $5-8 | Route handler + page.tsx + sub-component + role gate + audit-log row |
...
| Plan-review iter-1 (7 reviewers per frontmatter) | $7-10 | HIGH threat scope: security + multi-tenant + rls-auditor + database + ai-logic-tester + spec-checker + custodian |
| **Total estimated** | **$30-45** | At-or-below EXPANDED-SCOPE §10 $30-45 estimate. AT BOUNDARY of $50 per-plan halt gate (Rule 7d). |
...
**Slice-2 ceiling impact:** $30-45 of $200 Slice-2 ceiling consumed. Remaining: $155-170 for B-2b + B-3..B-7 (combined EXPANDED-SCOPE §10 estimate: $125-182).
...
2. **Down migration:** `mcp__supabase__apply_migration` against `00104_*.down.sql`. Drops columns/indexes/constraints; restores 00074 RPC body verbatim.
...
Per nwrp202 §17: this PLAN halts at plan-review iter-1 for Jake review. Do NOT auto-/nx.
...
- [ ] **§6.c index posture LOCKED per D-04** (full covering, no partial-WHERE-revoked_at-IS-NULL). NO partial-index escape per nwrp202 §4.
- [ ] **Plan-Author Latitude #3 disposition — `revoked_seq` column + re-invitation pattern** (see §2 Plan-Author Latitude #3 for Options A/B/C tradeoff matrix). Pick ONE:
  - [ ] **Option A accepted (plan-author default):** `revoked_seq INTEGER` column add; revoke increments seq; re-invitation creates new row with reset seq. Row identity survives re-invitation; audit trail preserved.
  - [ ] **Option B selected:** soft-delete-on-revoke + hard-delete-on-re-invite. No new column. Accepts soft-delete-rule exception trade for schema simplicity.
  - [ ] **Option C selected:** partial-index predicate `WHERE revoked_at IS NULL` re-introduced. Re-introduces timing oracle; nwrp202 §4 prohibition WAIVED + Jake explicit acceptance noted in SUMMARY.
- [ ] **Plan-Author Latitude #4 disposition — sliding-window UPDATE 90-day → 1-year in `submit_client_portal_message` + `mark_client_portal_message_read` RPCs** (see §2 Plan-Author Latitude #4). Pick ONE:
  - [ ] **In-scope accepted (plan-author default):** both RPC bodies authored inline in §5 Task 1 §3b + §3c with sliding-window line changed to `'1 year'`; consistent with nwrp200 LOCKED.
  - [ ] **Defer to follow-up plan (iter-1 may override):** accept inconsistent semantics post-B-2a ship (any token use rolls window back to 90 days). nwrp200 1-year LOCKED contradiction noted in SUMMARY.
- [ ] **§6.d token lifecycle change accepted.** 1-year default supersedes 00074 90-day (column default + RPC fallback). Sliding-window UPDATEs in `submit_client_portal_message` + `mark_client_portal_message_read` covered by Plan-Author Latitude #4 disposition checkbox above.
...
- [ ] **§6.g middleware PUBLIC_PATHS extension accepted.** Both `/owner/*` AND `/api/owner-portal/*`.
- [ ] **§6.h activity_log column add accepted.** `actor_token_id` column + LogActivityArgs extension + `'client_portal_access'` entity type + `'revoked'` action (plan-author proposed; iter-1 disposition).
- [ ] **§6.i draws.job_id index addition accepted.** change_orders.job_id verified existing.
...
- [ ] **Cost projection at $30-45** (§10) within $50 per-plan halt gate (Rule 7d). If mid-execute projection exceeds $50: halt-and-surface per Rule 7d, NO autonomous bump per Rule 7c.
...
3. **GATE B-2a HALT** — Jake reviews + ATTESTS the 8 EXPANDED-SCOPE §9 verification items. On approval: B-2b dispatch authorized. On rejection: iter-2 on B-2a OR fresh-session re-scope; B-2b dispatch held.
4. **B-2b dispatch readiness:** post-GATE-B-2a approval, B-2b becomes UNBLOCKED (depends_on: B-2a ship + GATE per nwrp200 charter).
...
**End of B-2a-PLAN.md.** Halts at plan-review iter-1 per `halt_after: true` and nwrp202 §17. Plan-author latitude items surfaced explicitly in §5.2 (out-of-scope items) and §2 (plan-author latitude) for iter-1 reviewer disposition.
