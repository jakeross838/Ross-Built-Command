...
<criteria>
mechanical:
  - "src/lib/knowledge-graph/ exists with subdirs validators/ + queries/; root contains README.md + index.ts + types.ts"
  - "src/lib/knowledge-graph/types.ts exports Validator<T>, ValidatorContext, ValidatorResult, ValidatorViolation types"
  - "src/lib/knowledge-graph/validators/ contains exactly 3 exemplar files: wi-001-inline-budget-context.ts, wi-013-multi-job-allocation.ts, client-pii-not-embedded.ts"
  - "src/lib/knowledge-graph/index.ts exports validatorRegistry; src/lib/knowledge-graph/queries/index.ts exports queryRegistry"
  - "__tests__/validators/ contains exactly 3 test files mirroring the 3 exemplars; each exercises ok=true AND ok=false paths"
  - "__tests__/_runner.ts recurses into __tests__/**/*.test.ts subdirectories (validates fix per nwrp172 ai-logic-tester F-H finding)"
  - "src/lib/types/database.types.ts exists and was generated by supabase gen types typescript --linked (CLI v2.99 emits export type Database = {...} type-alias form per AC-B1b-04 note); functionally equivalent to interface form for Tables<T> / InsertTables<T> / UpdateTables<T> generic resolution"
  - "src/lib/types/index.ts re-exports invoice.ts AI-parse types + named DB-row types (Tables<T>, InsertTables<T>, UpdateTables<T>, InvoiceRow, InvoiceInsert, InvoiceUpdate, ClientRow, ClientInsert, ClientUpdate)"
  - "src/lib/types/invoice.ts retains all 7 existing exports (no breaking change to existing 5 consumers)"
  - ".claude/hooks/nightwork-type-regen.sh exists with executable bit; wired into .claude/settings.json PreToolUse Bash chain; npx supabase fallback present per Rule 3 deviation"
  - "CLAUDE.md Development Rules contains 'Schema changes regenerate types' rule citing database.types.ts + the pre-commit hook"
  - "src/lib/verification/layer2/standards/integrity/ contains 4 standards files + self-test-helpers.ts; each standard registers via registerVerifyFn() and follows SKIP-clean canonical pattern from conservation/money-line-items-sum.ts"
  - ".planning/verification/standards/integrity/ contains 4 JSON rule files matching _schema.json shape; _schema.json domain enum extended to include 'integrity'"
  - "src/hooks/use-current-role.ts has second useEffect with onAuthStateChange listener gated by env flag NEXT_PUBLIC_AUTH_STATE_LISTENER === 'true' (strict string compare; early-return BEFORE supabase.auth.onAuthStateChange call); cleanup unsubscribes"
  - ".env.local.example documents NEXT_PUBLIC_AUTH_STATE_LISTENER (commented-out default)"
  - ".planning/tech-debt/TD-WB-LISTENER-UNFLAG.md exists with unflag path + observation criteria + rollback procedure"
dom: []
visual: []
behavioral:
  - "npx tsc --noEmit returns exit 0 (zero output) — all new types resolve cleanly"
  - "npm test enumerates the 3 validator test files in __tests__/validators/ subdirectory (post-runner-recursion fix); 23 validator test cases all PASS via the runner"
  - "Direct invocation npx tsx __tests__/validators/wi-001.test.ts returns 6/6 PASS; wi-013.test.ts returns 7/7 PASS; client-pii-not-embedded.test.ts returns 10/10 PASS"
  - "probeIntegrityStandards() from self-test-helpers.ts returns IntegrityProbeResult[] with all 4 integrity standards showing {verdict='SKIP', evidence-present=true, error-present=true} (non-vacuous SKIP)"
  - "Smoke harness post-execute run against Vercel preview returns ≤2 failures matching TD-WE-03 baseline (/financials/lien-releases + /financials/pay-apps DataGrid empty-state)"
  - "Pre-commit hook nightwork-type-regen.sh exits 0 on no-migrations-staged scenarios; exits non-zero with block reason when migration is staged but types are not (verified via synthetic test)"
  - "W.1 listener env-flag-off path verified by smoke harness (preview deploy without NEXT_PUBLIC_AUTH_STATE_LISTENER=true did not surface auth-state regressions in /today or other authenticated routes)"
semantic:
  - "Validator interface contract is async-uniform: every validator returns Promise<ValidatorResult> per Q3 nwrp153 — no sync overloads (locks the shape for F2-F5 35+ future validators)"
  - "3 exemplars exercise structurally distinct patterns: WI-001 cross-entity FK lookup; WI-013 multi-row aggregation; client-pii-not-embedded schema-shape policy enforcement (PII fence pattern per Q4 nwrp153)"
  - "ValidatorContext threads {org_id, supabase, user_id} per Q11 umbrella — validators have everything needed for org-scoped DB queries without trusting caller-provided org_id"
  - "ValidatorResult is {ok: boolean, violations: ValidatorViolation[]} with ValidatorViolation extracted as named exported type (Rule 2 deviation per executor; improves F2-F5 consumer ergonomics without breaking shape)"
  - "Layer 2 integrity standards follow conservation/money-line-items-sum.ts SKIP-clean canonical pattern: HEAD-probe data source; SKIP cleanly if unavailable; FAIL only on confirmed violation (no false-FAIL when introspection surface absent)"
  - "W.1 listener activation is env-flagged not unflagged in this slice (per Q5 amendment nwrp153) — production cutover requires explicit env-var addition + observation window per TD-WB-LISTENER-UNFLAG.md"
  - "Types pipeline is hybrid Q11 D: schema canonical in Supabase migrations; types auto-generated; validators in app — no manual-types-coexisting drift risk (CLAUDE.md rule + pre-commit hook prevent)"
</criteria>
...
- **AC-B1b-04:** `src/lib/types/database.types.ts` exists, contains `export type Database` OR `export interface Database` (CLI v2.99 emits type-alias form by default; both are functionally equivalent per cross-reviewer agreement at QA — Tables<T> / InsertTables<T> / UpdateTables<T> generics resolve correctly against both), and includes a `Tables: { clients: ... }` entry (proves B-1a applied). Verified by: `grep -E "export (type|interface) Database|Tables: \{" src/lib/types/database.types.ts` + `grep "clients:" src/lib/types/database.types.ts` non-empty.

   **AC text corrected per nwrp172 GATE 2 disposition:** original AC specified `grep "export interface Database"` which would fail against the type-alias form emitted by supabase CLI v2.99. Type alias form (`export type Database = { public: {...} }`) is functionally equivalent to interface form per cross-reviewer agreement at QA — Tables<T>/InsertTables<T>/UpdateTables<T> generics resolve correctly against both. Updated grep target accepts both forms.
...
supabase gen types typescript --linked > src/lib/types/database.types.ts
grep -E "^export (type|interface) Database" src/lib/types/database.types.ts   # ≥1 hit (accepts both supabase CLI v2.99 type-alias form + legacy interface form)
...
# During execute — validator tests (corrected post-nwrp172: project runner does not support --testPathPattern; run via direct npx tsx OR via npm test post-recursion-fix)
npm test                          # 23 validator cases PASS (post-runner-recursion-fix per nwrp172 ai-logic-tester F-H finding)
# OR equivalent direct invocation:
# npx tsx __tests__/validators/wi-001.test.ts                  # 6/6 PASS
# npx tsx __tests__/validators/wi-013.test.ts                  # 7/7 PASS
# npx tsx __tests__/validators/client-pii-not-embedded.test.ts # 10/10 PASS
