...
- **Pay Apps post-submit:** `revision_number` rows pattern. LOCKED + Rev N. This is the ONLY exception to "edits go through status_history transitions" — driven by AIA G702/G703 legal contract requirements.
...
**F1-Wave-B — Knowledge graph + auth (~2-3 weeks, foundational):**
- Plan B-1: Clients entity creation + `jobs.client_id` FK + backfill from embedded `client_*` columns + drop embedded columns + add `status_history` to clients. [Q2 A1 + Q12]
...
- Plan B-3: Soft-delete DB-trigger safety net (1 `SECURITY DEFINER` function + per-table triggers on every tenant table) + user-identity-threading convention (`current_setting('app.current_user_id', true)` populated by request middleware). [Q6 F]
...
- Plan B-5: Magic-link primitive (email-delivery + token-generation infrastructure for F4 reuse) + email-in receiving primitive (minimum — full ingestion rebuild Wave 3 per GAP item 21). [ARCHITECTURE §6 F1]
- Plan B-6: Cost-code wipe-and-reseed per D-020 + Q9 D fixture-maintenance contract codification + Layer 2 fixture-coverage assertion (CI hook). [D-035 #5 + Q9]
- Plan B-7: Knowledge-graph scaffold (`src/lib/knowledge-graph/` + 3-5 exemplar WI validators + `database.types.ts` generation pipeline via `supabase gen types` + pre-commit hook regenerating on migration changes + CLAUDE.md update for type-generation rule). [Q11 D]
- Plan B-8: RB org seed (`subscription_status='active'`, no Stripe customer ID, `trial_ends_at=null`) + Stripe test-mode sanity check + Q2 `EXPLAIN ANALYZE` verification step + `onAuthStateChange` listener activation (W.1 bridge cleanup) + harness extensions (multi-viewport per F1+-4 + behavioral test category). [Q10 A + Q2 verification + Q8 carry-forward + F1+-4]

**F1-Wave-C (optional; ~0.5-1 day, medium blast):**
- Plan C-1: `public.users` legacy retirement (refactor 5 src files to read `profiles + org_members` exclusively). Can absorb into Wave-A if grep-confirm goes clean.
...
- **Wave-B architectural commits warrant strategic checkpoints between plans:**
  ...
  - After Plan B-7 (knowledge-graph scaffold): Plan review of validator structure before B-8 RB seed.
- **Wave-C decision:** Jake decides at Wave-A plan-review whether to absorb into A or run separately.

### Dependencies between Plans

- A-1 (drop budgets) before B-1 (Clients) — both touch `budget_lines` references; sequence first.
- B-1 (Clients) before B-2 (`client_portal_access.client_id` FK) — FK depends on Clients table existing.
- B-3 (deletion trigger) after B-1 + B-2 — trigger applies to all new tables.
- B-4 (activity_log extension) parallel with B-5 (magic-link); both independent.
- B-7 (knowledge-graph scaffold + types pipeline) before B-8 (RB seed) — types pipeline needed for type-safe seed migration.
- A-3 (invoice_allocations org_id denormalize) independent — can land any time in Wave-A.

### Estimated total

- **Wave-A:** 1-2 days (autonomous-eligible post-plan-review).
- **Wave-B:** 2-3 weeks (per ARCHITECTURE §6 canonical estimate; 8 plans bundled into the foundational work).
- **Wave-C (optional):** 0.5-1 day (or absorb into A).
...
| Q11 | Knowledge graph implementation (library / app / DB / hybrid) | **RESOLVED** | **D — hybrid: schema/RLS in DB, types auto-generated, validators + helpers in app.** Schema canonical in Supabase migrations. Types via `supabase gen types typescript --linked > src/lib/types/database.types.ts`. Validators + queries in `src/lib/knowledge-graph/`. Matches existing Nightwork stack idioms. A premature (no second consumer); B loses schema/validator conceptual separation; C incompatible with WI complexity (WI-028 cross-entity in pure SQL unmaintainable). **F1 structure:** `src/lib/knowledge-graph/validators/wi-XXX.ts` (one per pattern), `queries/` (cross-entity helpers), `README.md`, `index.ts`. **Validator interface contract:** `(input: T, ctx: ValidatorContext) => ValidatorResult` where `ValidatorResult = { ok: true } \| { ok: false, reason, surface: 'block' \| 'flag' \| 'inform' }`. `ValidatorContext` threads org_id + supabase client. **F1 concrete deliverables:** (1) Type generation pipeline + pre-commit hook regenerates on migration changes + CI verifies types not stale (Drummond-gate-pattern). (2) Refactor `src/lib/types/invoice.ts` to extend `database.types.ts`. (3) Create `src/lib/knowledge-graph/` scaffold + README + index files + 3-5 exemplar WI validators (initial list: WI-028 punchlist-blocks-lien-release, WI-001 invoice-vendor-match, one Clients-related TBD-in-plan). (4) Cross-reference WORKFLOW-INTELLIGENCE.md patterns to implementation files (footer per pattern). (5) API route integration pattern: mutating routes call `validate(input, [validators], ctx)`; `block` failures → HTTP 422 with reasons; `flag`/`inform` returned in response metadata. **F2-F5 add bulk WI validators per their respective phase scope; F6 finalizes pay-app validators.** Failure mode mitigations: manual-types-coexisting prevented via CLAUDE.md rule + code review; schema-without-regen prevented via pre-commit + CI. Sub-decisions deferred to Plan phase: ValidatorContext shape, exemplar validator final selection, refactor pattern for existing API routes. |
| Q11 | Knowledge graph implementation (library / app / DB) | PENDING | — |
| Q12 | Entity versioning strategy | **RESOLVED** | **A — uniform existing pattern.** `status_history` JSONB + `activity_log` (Strategy 1-prime) + Pay Apps revision_number rows (LOCKED-after-submit only). No temporal tables, no snapshot columns, no per-entity variation. Existing pattern proven across 8 of 10 workflow entities. **F1 deliverables (mostly already in scope from earlier Qs):** (1) Add `status_history JSONB NOT NULL DEFAULT '[]'::jsonb` to: `lien_releases` (D-035 #2 + Q6), `jobs` (audit R.7 bonus + Q6), `clients` (new F1 entity per Q2 A1 normalize). (2) Update PATCH + bulk routes to append on transitions via existing `logStatusChange` pattern. (3) Document uniform pattern in CLAUDE.md Architecture Rules: "Every workflow entity has status_history JSONB; every cross-entity event goes through activity_log; Pay Apps + Draws use revision_number rows ONLY for LOCKED-after-submit legal/AIA contract requirements — this is the exception, not the rule." (4) Verify in F1 acceptance: all workflow entities have status_history populated on lifecycle transitions; Q9 fixture coverage + Q6 trigger safety net enforce coverage. **No new schema mechanism beyond gap closure.** |
...
- **F1+-4 (AC-1-12 Layer 3 ambiguity — multi-viewport harness):** folds into F1-Wave-B Plan B-8 harness extensions work.
...
- `src/middleware.ts:179 NODE_ENV → VERCEL_ENV` — already shipped during harness phase QA.
- WI-004 cost-code-per-line-item criterion — now reachable since 1.5b prototype-gallery merged to main; F1 plan-phase may select as 4th exemplar validator alongside WI-028 + WI-001 + Clients-related TBD.
...
|---|---|---|---|---|
| **F1-Wave-A** | Cleanup + D-035 + schema-shape concerns (drop budgets, drop change_order_budget_lines, add status_history, fix docx-html auth, invoice_allocations org_id denormalize, function search_path sweep, REVOKE EXECUTE sweep, extension schema move) | **4 plans** (A-1..A-4) + optional A-5 | **1-2 days** | Low |
| **F1-Wave-B** | Knowledge graph + auth foundation (Clients entity + normalize, Owner Portal Path A wiring, deletion safety net trigger, audit log extension, magic-link primitive + email-in primitive, cost-code reseed + fixture contract, knowledge-graph scaffold + types pipeline, RB seed + Stripe sanity + EXPLAIN ANALYZE + harness extensions) | **8 plans** (B-1..B-8) | **2-3 weeks** (~10-15 working days) | Foundational |
| **F1-Wave-C (optional)** | public.users legacy retirement (5 src files refactor) | **1 plan** (C-1, or absorb into A) | **0.5-1 day** | Medium (touches UI query results) |

**Total F1:** 12-13 plans across 2-3 waves; ~2.5-4 weeks calendar. Matches canonical ARCHITECTURE §6 estimate (3-4 weeks).

### Halt gates summary

- After Wave-A plan-review: Jake decides Wave-C absorption vs separate; greenlights Wave-A autonomous execution.
- After Plan B-3 (deletion safety net): manual verification of trigger firing on representative cases.
- After Plan B-7 (knowledge-graph scaffold): plan review of validator structure before B-8 RB seed.
- After Wave-B Plan B-8 complete: Q2 `EXPLAIN ANALYZE` verification gate — pass before Wave 1.1-Lite launch.
