...
criteria:
  ...
  semantic:
    - "Agent design: gsd-research-standards reusable across phases — caching strategy + TTL + authority hierarchy enable this without per-phase modifications (Plan-review watchpoint #3)"
    - "Agent design: gsd-fix-executor scope-narrowed to prevent agent-cascade — one failure, one commit, return; reduces blast radius vs spawning full gsd-executor"
```
