...
### Post-Phase-1 production interaction debug (nwrp247-254, 2026-05-28) — 2 additional lessons feeding the verification standard

Per `.planning/lessons.md` 2026-05-28 entry. The Phase-1 × W.1-listener interaction bug surfaced ~26h after Phase 1 SHIPPED. The bug required BOTH post-Phase-1 source AND `NEXT_PUBLIC_AUTH_STATE_LISTENER=true` on Production; truth-table confirmed via 4 real deploys. Every pre-ship verification surface (smoke harness, QA visual walk, Jake's initial binary test) ran on Vercel Preview or a Preview-equivalent surface; the bug lived ONLY on the Production code path. Mitigation: listener-off on Production. Mechanism investigation deferred to `TD-WB-LISTENER-PHASE1-INTERACTION.md`. Two additional lessons feed the tiering verification standard:

6. **Preview ≠ Production when env vars drive code paths.** Phase 1's QA report walked Vercel Preview deploys, which inherit env-var divergence relative to Production (NEXT_PUBLIC_VERCEL_ENV, the harness W.1 bridge gate in `src/lib/supabase/client.ts:69-105`, and potentially other env-flagged paths). The bug surfaced ONLY in Production's real-user code path. → This design's §3 SmokeAuthHelper already includes auth-aware smoke; **extend §3 + §2.x verification standard to REQUIRE per-phase enumeration of Preview/Production env-var divergence as an explicit coverage gap AND at least one authed verification step against an actual Production-target deploy (not Preview).** Codify this in the EXPANDED-SCOPE template (per §7 implementation surface) so every phase author lists the env-var divergence before /np dispatch.

7. **Env-var flips are deploy events needing post-flip authed verification.** The W.1 listener unflag (commit `aa3a5a4`, May 26 ~14:01) was framed as "operations-only; no source change." The post-flip Production-target deploy (a non-Phase-1 commit) was the LAST chance to catch the interaction in isolation before Phase 1 confounded the signal. Nobody walked it because the unflag was framed as low-risk infrastructure work. → **The tiered pipeline's standing rules MUST include an env-var-flip protocol:** (i) authed visual walk on the post-flip Production-target deploy, (ii) disk-evidence trail entry in MASTER-PLAN or a dedicated env-flip log, (iii) observation window documented with specific failure modes to watch for, (iv) if the flip enables a behavior path that wasn't previously exercised in Production (W.1 listener subscription path in this case), the walk MUST exercise that specific behavior, not just "look at the homepage." Sentry-alerting coverage doesn't substitute — a silent client-side deadlock (no thrown exception) never reaches Sentry; the AC-B4-07 3-detector spike threshold was correctly configured and correctly did not fire, because the bug was a HANG not an ERROR.

**Resolution surface:** lessons 6 + 7 feed the tiering design's verification standard. Implementation lands in the `tiering-implementation` phase (§12 Step 1) via:
- Extending `nightwork-init-phase.md` to require Preview/Production env-var divergence enumeration in the EXPANDED-SCOPE template (per L6).
- Extending `nightwork-qa.md` to require a Production-target authed walk (or explicit deferral with reasoning) as a tier-appropriate verification step (per L6).
- Adding a new env-var-flip protocol skill / command for change-event handling (per L7) — every Vercel/Supabase/secret-store env flip routes through the protocol before being declared "shipped."

The tiering-implementation phase remains parked at DRAFT EXPANDED-SCOPE commit `acbfa06`; on resume, these two requirements feed §7's implementation surface as touch-point additions. Sequencing per MASTER-PLAN §12 open ordering question (next session decision).

A third lesson (L8 — runtime truth-table over inference + verify experiment PREMISES not just results) is codified separately as CLAUDE.md Workflow-posture Rule 10. It applies to all diagnostic surfaces, not just tiering, so it doesn't extend this design — but it's the meta-lesson that made lessons 6 + 7 catchable in the first place.
