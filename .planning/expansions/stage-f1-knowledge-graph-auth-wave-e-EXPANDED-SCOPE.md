...
Items 1-7 unchanged. Items 8 + 9 resolved by Wave-E PASS. Item 10 **SATISFIED — D-080 codified in MASTER-PLAN.md** (per nwrp151 2026-05-15; decision matrix locked at 10 columns → auth.users, 1 column → profiles; mechanism A bulk migration in Wave-B Plan B-1 with pre-flight + execute-time orphan probes; working brief moved to `.planning/decisions-resolved/2026-05-15-user-identity-fk-convention-BRIEF.md`). Wave-B previously REVOKED until Wave-E GATE-N approved — Wave-E SHIPPED (commits 5958df0..0b6737a) + QA PASS (8 reviewers unanimous) + smoke gate green (11/13 PASS down to documented TD-WE-03). Wave-B now UNBLOCKED for Plan B-1 dispatch.

**Item 11 (SATISFIED with documented override; reframed 2026-05-15):** HARNESS_FIXTURE_PASSWORD rotation: Jake override per nwrp139 + nwrp141 reaffirmation. Account is RLS-scoped to synthetic fixture-harness-org (UUID `00000000-0000-0000-0000-fb1ce0a55e55`); transcript leak surface contained (the credential value never reached production-customer-data scope; the leaked transcript referenced a fixture-only account whose blast radius is bounded by RLS to synthetic harness data). Documented in calibration log (`.planning/lessons.md` 2026-05-15 entry covers the broader downstream-consumer-sweep meta-pattern; nwrp139 + nwrp141 capture the specific HARNESS_FIXTURE_PASSWORD decision history). Rotation deferred indefinitely OR until a triggering event — examples: (a) real customer data exposure on the harness account, (b) employee transition (someone with credential leaves), (c) RLS policy change that broadens harness-account access beyond fixture-harness-org, (d) GitHub Actions workflow secret model change. None of these triggers currently active; prereq SATISFIED via override. Earlier iter-2 patch framing (treated as outstanding work requiring rotation across 3 surfaces) is SUPERSEDED by this reframe.

**Item 12 (NEW; codified 2026-05-15 per nwrp147/148/149/150 post-Wave-E smoke calibration):** Smoke gate green except for documented TD entries. Real failures = 0. Post-Wave-E smoke calibration sequence (nwrp147 → nwrp150) resolved 12 logo-invariant failures (TD-WE-01 Tailwind arbitrary breakpoint fix + TD-WE-02 spec-vs-implementation top-right-vs-top-left correction) and 2 latent primary_ux selector failures (TD-WE-04 /today smoke selector stale from D-4 author time; TD-WE-05 /settings/workflow F3-deferred form). Current expected smoke state: 11/13 PASS. The 2 remaining failures are TD-WE-03 DataGrid empty-state indicators on /financials/lien-releases + /financials/pay-apps (documented; Wave 1.1-Lite scope). Wave-B Plan B-1 plan-review iter-1 must verify the smoke run at commit time still shows ≤2 failures AND all failures match the documented TD set (no NEW failure modes). If a new failure surfaces, halt + diagnose before Wave-B Plan B-1 dispatches.
