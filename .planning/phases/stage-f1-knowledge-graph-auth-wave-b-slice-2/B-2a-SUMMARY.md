...
| AC-B2a-08   | **PASS** (canonical 2026-05-22) | Forward-carried gate condition CLOSED at B-2b ship per nwrp213. Live revocation probe against real Production runtime (`https://nightwork-platform.vercel.app`): 5-step probe — create ephemeral row → GET `/owner/{token}` 200+dashboard → `UPDATE revoked_at = NOW()` → GET re-fetched 404+no dashboard → ephemeral row DELETED. Leaked-token kill-switch fires on real Vercel infrastructure (not just localhost). Cleanup confirmed. Full evidence: `.planning/qa-runs/2026-05-22-1145-stage-f1-wave-b-slice-2-B-2b-qa-report.md` (local; gitignored). |
