...
- **Cumulative Wave-D process gaps: 6 total.** iter-2-patch propagation (nwrp124, nwrp128, nwrp129) + precursor-violation scan (nwrp130) + precursor-scan regex precision (nwrp131) + precursor sub-category refinement (nwrp132). Pattern recognition: corrective waves disproportionately surface pre-existing tech debt because they touch files that have accumulated debt unscoped by the original feature that authored them. Each gap discovered late = a process-tightening at the next "before-dispatch" check. Pattern is converging — nwrp131 was a regex precision fix on the nwrp130 check; nwrp132 is a sub-category refinement on the nwrp131-tightened check, not a new category. Wave-B should ship with all 6 gaps closed via Rules 5-6 enforcement at plan-review iter-1 + the calibration-log-codified Wave-B adjustments (deliverable path reachability + precursor hook scan with sub-category distinction).
