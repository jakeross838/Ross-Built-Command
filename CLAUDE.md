...
- **Schema changes regenerate types.** Migrations under
  `supabase/migrations/` MUST be accompanied by a regenerated
  `src/lib/types/database.types.ts` in the same commit. Pre-commit hook
  `.claude/hooks/nightwork-type-regen.sh` blocks commits where migration
  files are staged but the regen output isn't (or is out of sync).
  Never edit `database.types.ts` by hand — it is generated by
  `supabase gen types typescript --linked` (or `npx supabase ...` if the
  CLI isn't on PATH) and any hand-edit is lost on next migration. New
  code consuming DB row shapes imports from `@/lib/types` (barrel) or
  `@/lib/types/database.types` directly; the existing
  `@/lib/types/invoice` AI-parse types remain untouched.
...
- **Q12 uniform versioning (codified F1-Wave-A Plan A-1).** Every workflow entity has `status_history JSONB` tracking `{who, when, old_status, new_status, note}` per transition; every cross-entity event goes through `activity_log` via `logActivity` helper; Pay Apps + Draws use `revision_number` rows ONLY for LOCKED-after-submit legal/AIA contract requirements — this is the exception, not the rule. As of Plan A-1, full workflow coverage: `change_orders`, `draws`, `invoices`, `proposals`, `purchase_orders`, `selections`, `draw_adjustments`, `job_milestones`, `lien_releases`, `jobs`. Both `status_history` (entity-local "how did THIS row reach its current state") and `activity_log` (cross-entity "what happened across the system") fire on a single status transition — that is BY DESIGN, not duplication. Do NOT extend `revision_number` pattern to non-AIA entities without explicit Jake authorization.
...
- **Stone blue palette + Slate type system + logo top-left.** The Slate design tokens are the Nightwork product palette — Stone Blue accent, Dark Slate, Warm Gray, White Sand. Exact hex values are pending visual reconciliation at CP2 (per CP1-RESOLUTIONS Q1 — both candidate sets render side-by-side at `/design-system/palette`; Jake picks visually). The Slate type system is **Space Grotesk** (display/headings, weights 400/500), **Inter** (body, 14-15px), **JetBrains Mono** (eyebrows, money, audit trails — 10-11px UPPERCASE) — NOT Calibri. (Calibri appeared in earlier project notes as a paraphrase; corrected 2026-04-29 per Stage 1.5a Q2=B. The actual implementation across `tailwind.config.ts`, `colors_and_type.css`, and the existing nightwork-design skill has always been Space Grotesk + Inter + JetBrains Mono.) Ross Built logo sits in the **top-left** of every authenticated surface (collapses to icon-only at <360px viewport per Q13). **Top-left is the productivity-app convention** (Gmail, Linear, Notion, Stripe Dashboard, Procore — every SaaS dashboard places the brand mark at top-left next to primary navigation). Earlier CLAUDE.md text said "top-right"; that was a category-error from SOW letterhead convention bleeding into the app UI spec — top-right logo placement is for documents and letterheads, NOT productivity apps. Implementation in `src/components/nav-bar.tsx:286` was always correct (brand mark inside `<Link href="/">` at the LEFT of the nav row); the spec was stale. Corrected 2026-05-15 per nwrp147 + TD-WE-02 disposition (Wave-E ship calibration). Hardcoded color hex codes, ad-hoc font-family declarations, bouncy easing, oversized rounded corners, purple/pink hues, dark glows, and logo placement drift are all blocked by hooks (per SYSTEM.md §13 Forbidden thresholds, mechanically enforced via `.claude/hooks/nightwork-post-edit.sh`).
