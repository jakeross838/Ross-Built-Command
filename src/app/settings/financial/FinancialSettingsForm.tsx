...
export default function FinancialSettingsForm({ org }: Props) {
  ...
  const [signatoryName, setSignatoryName] = useState(org.pay_app_signatory_name ?? "");
  const [signatoryTitle, setSignatoryTitle] = useState(org.pay_app_signatory_title ?? "");
    ...
    try {
      ...
      const res = await fetch("/api/organizations/current", {
        ...
        body: JSON.stringify({
          ...
          // R2 — blank stored as null so the print renders empty signature lines.
          pay_app_signatory_name: signatoryName.trim() || null,
          pay_app_signatory_title: signatoryTitle.trim() || null,
        }),
      });
      if (!res.ok) {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <Section title="Pay App Signatory">
        <p className="text-[12px] text-[color:var(--text-secondary)] leading-relaxed">
          Printed on the AIA G702 pay-app certification signature line. Leave
          blank to print empty lines for a wet signature — the pay app never
          fills in a default name.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-1">
          <TextField
            ...
            placeholder="e.g. Jake Ross"
          />
          <TextField
            label="Signatory Title"
            value={signatoryTitle}
            onChange={setSignatoryTitle}
            placeholder="e.g. Director of Construction"
          />
        </div>
      </Section>
