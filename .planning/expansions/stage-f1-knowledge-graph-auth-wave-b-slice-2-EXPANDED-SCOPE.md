...
| **B-3 per-plan halt gate fires mid-authoring (estimate sits at $35-50 boundary; Rule 7d threshold)** | Pre-mortem fall-back per nwrp169 §107: SPLIT B-3 into B-3a (trigger function + DEF-WC-1 RESTRICTIVE backstop) + B-3b (DEF-WC-3 ARCHITECTURE.md RLS posture summary table — the ~1hr authoring step). B-3a stays at threat-model HIGH with security + multi-tenant + rls + database reviewers; B-3b is pure documentation work with custodian + spec-checker only. Plan-author surfaces if mid-authoring spend projects beyond $50; Jake authorizes split OR re-scope. NOT a scope-engineering-to-dodge-gate per Rule 7e — B-3a + B-3b have genuinely different review surfaces + atomic-rollback profiles. Reference: nwrp169 KNOWN-RISK FLAG. |
