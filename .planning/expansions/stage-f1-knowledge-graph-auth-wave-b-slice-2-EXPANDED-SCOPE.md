...
3. **B-4 — `activity_log.entity_type` TS union extension + write-site coverage + MEDIUM-1 + TD-B1abis-01 + TD-B1abis-02 + W.1 listener unflag (conditional) + B-1b QA HIGH-findings bundle** [Q6 F + Slice-1 carry-forwards per nwrp166 §10-11 + B-1b QA carry-forwards per nwrp172 GATE 2 disposition]
   ...
   - **Carry-forward 5 (per nwrp172 GATE 2 disposition; B-1b QA HIGH-findings bundle):** F-J + F-K NaN guards in WI-013 + WI-001 validators. Add `Number.isFinite()` defensive guards on `amount` (WI-013) and `total_amount` (WI-001) aggregation steps. Each ~4 lines; trivial fix. Without guards, NaN inputs silently produce ok=true (sum-drift / overage checks skip via JS NaN-comparison semantics). New violation codes: `wi-013-allocation-amount-not-finite` + `wi-001-invoice-amount-not-finite`. 2 unit test cases per validator.
   - **Carry-forward 6 (per nwrp172 GATE 2 disposition; B-1b QA HIGH-findings bundle):** F-D WI-013 jobs lookup direct-filter rationale header comment. Validator's `.from("jobs").select("id, org_id").eq("id", alloc.job_id).maybeSingle()` intentionally lacks `.eq("org_id", ctx.org_id)` for diagnostic clarity (validator must SEE cross-tenant rows to emit `wi-013-allocation-cross-tenant` rather than `job-not-found`). Outcome-safe under RLS-bound callers but technically deviates from CLAUDE.md "Multi-tenant RLS BY CONSTRUCTION" — make the rationale explicit via inline header comment. Pure docs change.
   - **Carry-forward 7 (per nwrp172 GATE 2 disposition; B-1b QA MEDIUM finding):** F-A client-pii-not-embedded regex case-sensitivity fix. Add `i` flag to all 4 patterns (WILDCARD_PATTERN, ALIAS_WILDCARD, EMAIL_PATTERN, PHONE_PATTERN). `Clients(email)` / `CLIENTS(*)` etc. currently slip past undetected. PostgREST is case-sensitive at runtime so production impact low, but developer typos slip the fence. 4-character fix; no false-positive risk because patterns are anchored at `clients?\s*\(`. 2 unit test cases.
   - Threat model: low (additive type extension + write-site sweep + one application-layer defense-in-depth fix + one bounded-retry catch + one trigram index migration + one env-var addition + 2 trivial NaN guards + 1 header comment + 4-character regex flag).
...
| **B-3 per-plan halt gate fires mid-authoring (estimate sits at $35-50 boundary; Rule 7d threshold)** | **IF the per-plan halt gate fires** (B-3 mid-authoring spend projects beyond $50 per Rule 7d), plan-author halts + surfaces to Jake. Jake picks one of the following non-exhaustive fall-back options (NOT pre-decided in this EXPANDED-SCOPE; plan-author surfaces, Jake selects at /np time when the gate fires): (1) **SPLIT** B-3 into B-3a (trigger function + DEF-WC-1 RESTRICTIVE backstop, threat-HIGH, full reviewer set) + B-3b (DEF-WC-3 ARCHITECTURE.md RLS posture summary table, docs-only, custodian + spec-checker only). NOT scope-engineering-to-dodge-gate per Rule 7e — B-3a + B-3b have genuinely different review surfaces + atomic-rollback profiles. (2) **DEFER DEF-WC-3 entirely** to a standalone Slice-3 docs phase (closes DEF-WC-3 separately; B-3 stays atomic trigger work only). (3) **TRIM DEF-WC-3 to phased table per Q12 alternative** (cover only Slice-1/Slice-2-touched entities + note remaining tables for Wave-B0/F2 — reverses Q12's full-table recommendation under cost pressure). (4) **HALT B-3 entirely** + defer to Wave-B0 fallback wave per nwrp152 (Wave-B0 was the original umbrella escape valve for >2.5-day plans; same option-space applies here). (5) **DROP DEF-WC-1 RESTRICTIVE backstop** to a separate cleanup wave (B-3 ships trigger + DEF-WC-3 only; DEF-WC-1 deferred). Decision authority: plan-author surfaces with cost projection + scope-reduction options; Jake picks the fall-back. Reference: nwrp169 KNOWN-RISK FLAG; nwrp170 audit clarification (option-space preserved, no single fall-back pre-decided). |
